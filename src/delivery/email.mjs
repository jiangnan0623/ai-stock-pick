...
const briefAnalysis=item=>[
  ...
  item.theme_analysis?.breadth?`题材最新日涨停${item.theme_analysis.breadth.latest_stocks}只、首板${item.theme_analysis.breadth.latest_first_boards}只，10日覆盖${item.theme_analysis.breadth.stocks}只`:null,
  item.theme_analysis?.authenticity?.level==='direct'?`主营匹配：${item.theme_analysis.authenticity.matched_keywords.join('、')}`:'主营正宗性未核验',
