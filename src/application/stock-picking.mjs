import {ymd,daysAgoYmd,dateValue,normalizeCode,normalizeBars,isMarketTimestampFresh,isoDateLike,limitThreshold,toTsCode,normalizeTheme,buildThemeStats,assessThemeAuthenticity,scoreThemeStructure,marketCapFitScore,isTechnicalSetupEligible,isRecommendationEligible,isPaperCandidateEligible,hasVerifiedCatalyst,isChiNextStock,mapLimit} from '../domain/market-rules.mjs'
import {aStockLimitPool,aStockTencentQuotes,aStockTencentKline,aStockEastmoneyKline,aStockBaiduKline,aStockThsLimitReasons} from '../infrastructure/providers/a-stock-data.mjs'

export function createStockPickingService({config,clients}){
  const {tushare,xiaoshi,eastmoneyQuote,eastmoneyKline,eastmoneyCompanyProfile,eastmoneyMinute,serpNews}=clients
  ...
  async function recommendations(){
    ...
    const result=await mapLimit(rows,20,async x=>{
      ...
      const paperPullback=Boolean(priorLimit)&&shortWindow&&pullbackPct>=2&&pullbackPct<=18&&recoveryRatio>=.30&&price<=limitPeak*1.03
      ...
      const paperLiquid=Number.isFinite(avgAmount3)&&avgAmount3>=80000000
      const klineVerified=closes.length>=20
      const technicalPass=isTechnicalSetupEligible({poolVerified,recentLimitUp:recent10,klineVerified,pullback:paperPullback,liquid:paperLiquid,quoteFresh:quoteOk})
      if(technicalPass){
      ...
      const eventVerified=hasVerifiedCatalyst({announcementEvidence,newsEvidence})||reasonEventVerified; const theme=Boolean(poolThemeEvidence||eventVerified)
      ...
      const themeStructureScore=scoreThemeStructure(themeBreadth);const capScore=marketCapFitScore(quote?.float_mcap_yi); const openTimes=Number(x.open_times); const themeWidthOk=themeBreadth.latest_stocks>=2; const themeScore=theme?10+(authenticity.level==='direct'?5:0)+(themeWidthOk?5:0)+themeStructureScore:0; const pullbackScore=paperPullback?(pullback?20:12)+(volumeContracted?5:0):0; const amountScore=paperLiquid?(liquid?10:6):0; const scoreBreakdown={theme_event:themeScore,pullback_rebound:pullbackScore,stock_character:Math.min(20,8+limitCount*4+(Number.isFinite(openTimes)?Math.max(0,4-openTimes):0)),liquidity_market_cap:paperLiquid?amountScore+capScore:0,market_confirmation:quoteOk&&intradayFresh?15:0}
      ...
      const themeAuthentic=authenticity.level==='direct'
      const eligible=isRecommendationEligible({technicalPass,total,themeEvidence:Boolean(poolThemeEvidence),themeAuthentic,intradayFresh,validPlan:hasValidPlan})
      const paperEligible=isPaperCandidateEligible({technicalPass,total,themeEvidence:Boolean(poolThemeEvidence),authenticity:authenticity.level,intradayFresh,validPlan:hasValidPlan})
      return {code:x.ts_code,name:quote?.name||x.name||x.ts_code,price:quoteOk?price:null,limit_up_count:limitCount,limit_up_dates:limitDates.length?limitDates:(x.pool_limit_dates||[]),score:total,score_breakdown:scoreBreakdown,status:eligible?'推荐':'观察',paper_eligible:paperEligible,risk_level:eligible?'中高':null,max_position_pct:eligible?10:null,source,providers,trade_date:date,quote_timestamp:quote?.observed_at||quote?.timestamp||null,theme_evidence:poolThemeEvidence||(announcementEvidence?{type:'announcement',title:announcementEvidence.title,time:announcementEvidence.publish_time||announcementEvidence.date||null,source:providers.announcements}:newsEvidence?{type:'news',...newsEvidence}:null),theme_analysis:{breadth:themeBreadth,authenticity,structure_score:themeStructureScore,is_new_cluster:themeBreadth.latest_first_boards>=2&&themeBreadth.first_seen===themeBreadth.latest_date},related_news:realtimeNews,checks:{pool:poolVerified?'近10交易日完整池':'候选池不完整',kline:klineVerified?'至少20根日线已核验':'日线不足20根',recent_limit_up:recent10?'已核验':'近10日无涨停',pullback:paperPullback?'中度收紧回调门槛通过（修复≥30%）':(priorLimit?'涨停后回调窗口/幅度/修复不通过':'只有当日涨停，尚无板后回调'),volume:volumeContracted?'回调阶段缩量':'回调缩量未确认',liquidity:paperLiquid?'中度收紧成交额门槛通过（≥8000万元）':'近3日成交额不足或缺失',market_cap:capScore>=3?'流通市值适配':'流通市值偏离或缺失',theme:themeAuthentic?'主营直接覆盖核心题材':(authenticity.business_available?'主营资料存在但未直接覆盖核心题材':'主营资料缺失'),theme_breadth:themeBreadth.latest_stocks>=2?'题材最新日宽度通过':'题材宽度不足（风险提示，不否决）',theme_structure:themeStructureScore===5?'新题材首板集群':(themeBreadth.max_board>=2?'存在连板梯队':'普通题材宽度'),quote:quoteOk?'行情新鲜':'行情缺失或过期',intraday:intradayFresh?'当日分时新鲜':'分时缺失或过期'},technical:{bars:closes.length,prior_limit_date:priorLimit?.date||null,limit_up_price:Number.isFinite(limitPeak)?limitPeak:null,post_limit_days:postLimitBars.length,pullback_low:Number.isFinite(postLimitLow)?postLimitLow:null,pullback_pct:Number(pullbackPct.toFixed(2)),recovery_ratio:Number((recoveryRatio*100).toFixed(2)),volume_contracted:volumeContracted,avg_amount_3d:Number.isFinite(avgAmount3)?Number(avgAmount3.toFixed(0)):null,float_market_cap_yi:Number.isFinite(Number(quote?.float_mcap_yi))?Number(quote.float_mcap_yi):null,minute_bars:minute.length},missing_fields:[...(poolVerified?[]:['完整近10交易日涨停池']),...(klineVerified?[]:['至少20根日线']),...(recent10?[]:['近10日涨停历史']),...(paperPullback?[]:['涨停后1-5日短回调与至少30%修复']),...(volumeContracted?[]:['回调缩量']),...(paperLiquid?[]:['近3日日均成交额≥8000万元']),...(theme?[]:['涨停附近的题材/触发事件']),...(authenticity.business_available?[]:['主营业务资料']),...(themeAuthentic?[]:['主营直接覆盖核心题材']),...(quoteOk?[]:['新鲜行情快照']),...(intradayFresh?[]:['新鲜1分钟分时'])],trade_plan:hasValidPlan?{entry_trigger:'中度收紧标准；仅在价格进入计划区间、回踩不破修复位且板块未退潮时确认，单只仓位不超过10%',entry_zone_low:entryLow,entry_zone_high:entryHigh,entry_reference:entryReference,take_profit_1:takeProfit1,take_profit_2:takeProfit2,stop_loss:stopLoss,risk_reward_1:Number.isFinite(entryRisk)&&entryRisk>0?Number(((takeProfit1-entryReference)/entryRisk).toFixed(2)):null,risk_reward_2:Number.isFinite(entryRisk)&&entryRisk>0?Number(((takeProfit2-entryReference)/entryRisk).toFixed(2)):null,invalidation:['跌破涨停后回调低点','行情或分时过期','近3日流动性跌破8000万元','题材证据被证伪','板块退潮']} : null}
    })
    const rankedRecommendations=result.filter(x=>x.status==='推荐').sort((a,b)=>b.score-a.score)
    const recommendations=rankedRecommendations.slice(0,3)
    const recommendationCodes=new Set(recommendations.map(x=>x.code))
    const paperCandidates=result.filter(x=>x.paper_eligible&&!recommendationCodes.has(x.code)).sort((a,b)=>b.score-a.score).slice(0,3)
    const paperCodes=new Set(paperCandidates.map(x=>x.code))
    return {date,source,fallback,fallbackReason,poolVerified,total:rows.length,recommendations,paperCandidates,watch:result.filter(x=>x.status==='观察'&&!paperCodes.has(x.code)).sort((a,b)=>b.score-a.score).slice(0,12),updatedAt:new Date().toISOString(),strategy:'ai-stock-pick-v7-high-risk-moderate'}
  }
  return {recommendations,aStockPrimaryPool}
}
