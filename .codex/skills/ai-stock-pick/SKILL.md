...
- The deployed `ai-stock-pick-v7-high-risk-moderate` gate runs slightly below the full spec: pullback recovery ≥30%, latest-3-session average turnover ≥CNY 80 million, recommendation score ≥75, and **direct main-business coverage of the core theme as a hard gate**. Theme breadth (≥2 stocks on the theme's latest date) and announcement/news catalysts are scoring and risk flags, not hard gates. Keep these numbers in sync with `server.mjs` and `src/domain/market-rules.mjs`; do not silently lower them.
