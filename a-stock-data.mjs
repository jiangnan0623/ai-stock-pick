...
async function eastmoneyFetch(url){
  const run=eastmoneyQueue.then(async()=>{const delay=1100-(Date.now()-lastEastmoneyCall);if(delay>0)await wait(delay);try{return await remoteFetch(url,{headers:{'User-Agent':UA,Referer:'https://quote.eastmoney.com/'}})}finally{lastEastmoneyCall=Date.now()}})
...
export async function aStockTencentQuotes(codes){
  ...
  const query=normalized.map(prefix).join(','); const r=await remoteFetch(`https://qt.gtimg.cn/q=${query}`,{headers:{'User-Agent':UA}}); if(!r.ok)throw Error(`a-stock-data腾讯行情 HTTP ${r.status}`)
...
export async function aStockTencentKline(code,limit=30,adjust='qfq'){
  ...
  const r=await remoteFetch(`https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?${q}`,{headers:{'User-Agent':UA,Referer:'https://gu.qq.com/'}});if(!r.ok)throw Error(`a-stock-data腾讯K线 HTTP ${r.status}`)
...
export async function aStockBaiduKline(code){
  ...
  const r=await remoteFetch(`https://finance.pae.baidu.com/selfselect/getstockquotation?${q}`,{headers:{'User-Agent':UA,Accept:'application/vnd.finance-web.v1+json',Origin:'https://gushitong.baidu.com',Referer:'https://gushitong.baidu.com/'}});if(!r.ok)throw Error(`a-stock-data百度K线 HTTP ${r.status}`)
...
export async function aStockThsLimitReasons(date){
  ...
  const r=await remoteFetch(`https://data.10jqka.com.cn/dataapi/limit_up/limit_up_pool?${q}`,{headers:{'User-Agent':UA}});if(!r.ok)throw Error(`a-stock-data同花顺涨停原因 HTTP ${r.status}`)
