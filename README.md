...
3. 以涨停原因第一项作为核心题材，将其余项目视为叠加概念；**推荐硬门槛**要求通过 Tushare `stock_company` 核验核心题材由主营**直接覆盖**，子公司、间接关联或主营资料缺失时降级观察。题材最新出现日≥2 只股票共同涨停作为题材宽度评分与风险提示（不否决）。
4. 检查回调是否缩量，要求近 3 日日均成交额至少 8000 万元（正式规格为 1 亿元）；流通市值 20–200 亿元获得最高适配分，10–300 亿元获得次级分。综合题材、股性、流动性、行情快照和 1 分钟分时确认。
5. 最多推荐 3 只；未通过全部硬门槛时只列观察，不凑数。通过更高分门槛（总分 ≥ 78）但未进入正式推荐前 3 名的标的，单独写入 `paperCandidates`（高分观察/纸面候选）。
> 实际部署推荐档：涨停池完整 + 近 10 日有涨停 + 日线≥20 根 + **回调修复 ≥ 30%** + **近 3 日日均成交额 ≥ 8000 万元** + 题材证据存在 + 主营资料具备且**直接覆盖核心题材** + 当日分时新鲜 + 交易计划有效，且**五维总分 ≥ 75**。题材宽度不足、公告/新闻缺失仅作为评分与风险提示（不否决）；主营未直接覆盖（子公司/间接关联/缺失）直接降级观察。
